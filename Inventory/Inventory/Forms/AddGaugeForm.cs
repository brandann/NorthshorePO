    ...
    public partial class AddGaugeForm : Form
    {
        ...
        private void Submit(string thickness, string type)
        {
            try
            {
                ...
                command.CommandText = "INSERT INTO Thickness (THICKNESS, UNITS) VALUES('" + thickness + "', '" + type + "')";

                connection.Open();
                command.ExecuteNonQuery();

                connection.Close();
                MessageBox.Show("Gauge Inserted");
            }
            catch (Exception ex)
            {
